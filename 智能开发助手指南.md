...
```bash
# 1. 复制基础项目结构
cp -r mcp-context mcp-file-manager
cd mcp-file-manager
...
**大模型应该：**
1. **分析现有代码结构** - 参考 `mcp-context/index.js`
...
**生成的代码结构：**
```javascript
// 基于 mcp-context 的代码结构，替换为文件管理逻辑
...
**大模型执行流程：**
1. **复制项目**：`cp -r mcp-context mcp-database-manager`
...
**大模型执行流程：**
1. **复制项目**：`cp -r mcp-context mcp-task-scheduler`
2. **更新配置**：修改项目信息
