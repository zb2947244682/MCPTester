...
xcopy mcp-context-memory mcp-file-manager /E /I /H
cd mcp-file-manager
# 如果存在.git目录，需要删除
...
要求：
1. 自动复制mcp-context-memory项目结构（注意删除.git目录）
2. 重命名为mcp-[功能名]
...
**大模型执行流程：**
1. **复制项目**：`xcopy mcp-context-memory mcp-database-manager /E /I /H`
...
**大模型执行流程：**
1. **复制项目**：`xcopy mcp-context-memory mcp-task-scheduler /E /I /H`
2. **更新配置**：修改项目信息
